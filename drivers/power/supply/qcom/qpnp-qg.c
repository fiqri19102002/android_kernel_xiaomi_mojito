				...
				*soc = pre_soc;
				...
				if (ibat_count >= 3) {
					...
					if (prop.intval) {
						...
						*soc = pre_soc;
						qg_dbg(chip, QG_DEBUG_STATUS, "BQ charging,not update 100%!\n");
					}
					ibat_count = 0;
				}
			}
...
#ifdef CONFIG_MACH_XIAOMI_MOJITO
			if (((chip->recharge_soc == DEFAULT_RECHARGE_SOC) || 
			     (chip->msoc > chip->recharge_soc + 2)) && (chip->msoc != 100)) {
...
static int qg_parse_dt(struct qpnp_qg *chip)
{
	...
	if (size) {
		...
		if (chip->dt.dec_rate_seq) {
			...
			of_property_read_u32_array(node, "qcom,soc_decimal_rate",
	...
	pr_err("size=%d dt.dec_rate_len=%d [0]=%d [end]=%d\n",
	        size, chip->dt.dec_rate_len, chip->dt.dec_rate_seq[0], chip->dt.dec_rate_seq[chip->dt.dec_rate_len - 1]);
#endif
